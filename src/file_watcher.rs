...
impl SessionFileWatcher {
    ...
    pub fn new(encoded_project_map: HashMap<String, PathBuf>) -> Result<Self, notify::Error> {
        ...
        std::thread::spawn(move || {
            Self::process_events(sync_rx, event_tx_clone, sessions_dir_clone, encoded_project_map_clone);
        });
    ...
    pub fn start_watching(&mut self) -> Result<(), notify::Error> {
        ...
        if !sessions_dir.exists() {
            if let Err(e) = std::fs::create_dir_all(&sessions_dir) {
                eprintln!("Failed to create sessions directory {:?}: {}", sessions_dir, e);
            }
        }
            ...
            match result {
                Ok(event) => {
                    ...
                    for path in &event.paths {
                        ...
                        if let Some((project_path, session_id)) = Self::parse_session_path(path, &sessions_dir, &encoded_project_map) {
                            let file_event = match event.kind {
                                EventKind::Create(_) => Some(SessionFileEvent::SessionFileCreated {
                                    project_path,
                                    session_id,
                                    file_path: path.clone(),
                                }),
                                EventKind::Modify(_) => Some(SessionFileEvent::SessionFileModified {
                                    project_path,
                                    session_id,
                                    file_path: path.clone(),
                                }),
...
mod tests {
    ...
    fn test_parse_session_path_with_lookup_map() {
        ...
        let result = SessionFileWatcher::parse_session_path(&file_path, &sessions_dir, &encoded_map);
