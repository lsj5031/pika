# AGENTS.md - Pika (Pi Agent Manager)
...
- **State**: `AppState` combines `WSState`, `ApiState`, `ProcessManager`
- **Tests**: Integration tests in `tests/`, use `pika::create_test_app()` with `test-utils` feature

## Code Style
- Rust: Use `thiserror` for errors, async/await with Tokio, Arc<Mutex<>> for shared state
